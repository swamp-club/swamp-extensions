...
Deno.test("markDirty(relPath): directory dirty path walks into files and pushes them", async () => {
  const cachePath = await Deno.makeTempDir({ prefix: "s3sync-dir-dirty-" });
  try {
    const mock = createMockS3Client();
    mock.storage.set(".datastore-index.json", encodeIndex({}));
    const service = new S3CacheSyncService(mock, cachePath);
...
Deno.test("markDirty(relPath): deleted directory removes all entries under prefix from index", async () => {
  const cachePath = await Deno.makeTempDir({ prefix: "s3sync-dir-deleted-" });
  try {
    const mock = createMockS3Client();
    mock.storage.set(".datastore-index.json", encodeIndex({}));
    const service = new S3CacheSyncService(mock, cachePath);
...
Deno.test("markDirty(relPath): scoped push falls back to full walk when dirty paths resolve to nothing", async () => {
  const cachePath = await Deno.makeTempDir({ prefix: "s3sync-fallback-" });
  try {
    const mock = createMockS3Client();
    mock.storage.set(".datastore-index.json", encodeIndex({}));
    const service = new S3CacheSyncService(mock, cachePath);
