...
// ==========================================================================
// swamp-club#2242: the pullChanged bulk-diff listing is namespace-scoped, and
// a self-authored scoped push re-arms the commitSeq fast path instead of
// permanently disarming it. assertDatastoreExportConformance covers the
// export contract, not listing scope or index completeness, so these are
// direct assertions. Mirrors the S3 suite.
// ==========================================================================

/** Wraps listAllObjects so a test can see which prefixes were listed. */
function recordListPrefixes(
  mock: ReturnType<typeof createMockGcsClient>,
): (string | undefined)[] {
  const seen: (string | undefined)[] = [];
  const inner = mock.listAllObjects.bind(mock);
  (mock as unknown as Record<string, unknown>).listAllObjects = (
    subPrefix?: string,
    signal?: AbortSignal,
  ) => {
...
Deno.test("swamp-club#2242: pullChanged slow path lists only the namespace prefix", async () => {
  const mock = createMockGcsClient();
  const cachePath = await Deno.makeTempDir({ prefix: "gcssync-2242-list-" });
  try {
    mock.storage.set(
      "my-ns/.datastore-index.json",
      encodeIndex({
    ...
    for (let i = 0; i < 50; i++) {
      mock.storage.set(`other-ns/data/model/${i}/raw`, new Uint8Array([1]));
    }

    const subPrefixes = recordListPrefixes(mock);
    const svc = new GcsCacheSyncService(mock, cachePath);
    await svc.pullChanged({ namespace: "my-ns" });
...
Deno.test("swamp-club#2242: root-key entries survive the scoped listing", async () => {
  const mock = createMockGcsClient();
  const cachePath = await Deno.makeTempDir({ prefix: "gcssync-2242-root-" });
  try {
    mock.storage.set(
      "my-ns/.datastore-index.json",
      encodeIndex({
    ...
    mock.storage.set("data/legacy/1/raw", new TextEncoder().encode("legacy"));
    for (let i = 0; i < 20; i++) {
      mock.storage.set(`other-ns/data/x/${i}/raw`, new Uint8Array([1]));
    }
...
Deno.test("swamp-club#2242: one failed prefix listing skips pruning entirely", async () => {
  const mock = createMockGcsClient();
  const cachePath = await Deno.makeTempDir({ prefix: "gcssync-2242-partial-" });
  try {
    mock.storage.set(
      ".datastore-index.json",
      encodeIndex({
    ...
    mock.storage.set("data/live/1/raw", new TextEncoder().encode("live"));
    ...
    assertEquals(
      mock.gets.filter((k: string) => k === "config/stale.yaml").length,
      1,
...
Deno.test("swamp-club#2242: scoped push re-arms the sidecar commitSeq", async () => {
  const cachePath = await Deno.makeTempDir({ prefix: "gcssync-2242-push-" });
  try {
    const mock = createMockGcsClient();
    const ts = new Date().toISOString();
    seedV2Repo(mock, {
...
Deno.test("swamp-club#2242: scoped push keeps untouched shards in the on-disk index", async () => {
  const cachePath = await Deno.makeTempDir({ prefix: "gcssync-2242-merge-" });
  try {
    const mock = createMockGcsClient();
    const ts = new Date().toISOString();
    seedV2Repo(mock, {
...
Deno.test("swamp-club#2242: a failed index merge leaves the fast path unarmed", async () => {
  const cachePath = await Deno.makeTempDir({ prefix: "gcssync-2242-nomerge-" });
  try {
    const mock = createMockGcsClient();
    const ts = new Date().toISOString();
    seedV2Repo(mock, {
...
Deno.test("swamp-club#2242: pull after a scoped push hits the commitSeq fast path", async () => {
  const cachePath = await Deno.makeTempDir({ prefix: "gcssync-2242-rt-" });
  try {
    const mock = createMockGcsClient();
    const ts = new Date().toISOString();
    seedV2Repo(mock, {
    ...
    const listed = recordListPrefixes(mock);
...
Deno.test("swamp-club#2242: scoped commitPush keeps untouched shards and re-arms", async () => {
  const cachePath = await Deno.makeTempDir({ prefix: "gcssync-2242-commit-" });
  try {
    const mock = createMockGcsClient();
    const ts = new Date().toISOString();
    seedV2Repo(mock, {
...
Deno.test("swamp-club#2242: a concurrent commit must not re-arm through a stale index merge", async () => {
  const cachePath = await Deno.makeTempDir({ prefix: "gcssync-2242-stale-" });
  try {
    const mock = createMockGcsClient();
    const ts = new Date().toISOString();
    seedV2Repo(mock, {
...
Deno.test("swamp-club#2242: a bulk-invalidated push must not re-arm over missing local files", async () => {
  const cachePath = await Deno.makeTempDir({ prefix: "gcssync-2242-bulk-" });
  try {
    const mock = createMockGcsClient();
    const ts = new Date().toISOString();
    seedV2Repo(mock, {
...
async function pullFresh2353(
  ...
  const cachePath = await Deno.makeTempDir({ prefix: "gcssync-2353-pull-" });
  try {
    await new GcsCacheSyncService(mock, cachePath).pullChanged();
...
for (const type of TYPE_DEPTHS) {
  ...
  Deno.test(`swamp-club#2353: gc of version directories deletes their objects and shard entries (type ${typeName})`, async () => {
    const cachePath = await Deno.makeTempDir({ prefix: "gcssync-2353-gc-" });
    try {
      const mock = createMockGcsClient();
      ...
      const modelShard = GcsCacheSyncService.partitionKeyFromPath(
        `${dir}/1/raw`,
      )!;
  ...
  Deno.test(`swamp-club#2353: data delete of a whole data name removes every object (type ${typeName})`, async () => {
    const cachePath = await Deno.makeTempDir({ prefix: "gcssync-2353-del-" });
    try {
      const mock = createMockGcsClient();
      const { dirs, files } = await seedDataName2353(
        mock,
...
Deno.test("swamp-club#2353: a model id that extends another at a segment boundary is not read", async () => {
  const cachePath = await Deno.makeTempDir({ prefix: "gcssync-2353-bound-" });
  try {
    const mock = createMockGcsClient();
    const { dirs, files } = await seedDataName2353(
      mock,
...
Deno.test("swamp-club#2353: a dirty type directory does not read a type that extends its name", async () => {
  const cachePath = await Deno.makeTempDir({ prefix: "gcssync-2353-type-" });
  try {
    const mock = createMockGcsClient();
...
Deno.test("swamp-club#2353: deleting one version reads only its type and model shards (#1913)", async () => {
  const cachePath = await Deno.makeTempDir({ prefix: "gcssync-2353-reads-" });
  try {
    const mock = createMockGcsClient();
...
Deno.test("hydrateFile with namespace: writes where core reads, one GET (swamp-club#2404)", async () => {
  const cachePath = await Deno.makeTempDir({ prefix: "gcssync-hydrate-ns-" });
  try {
    const mock = createMockGcsClient();
    mock.storage.set(
      "my-ns/data/m/1/raw",
...
Deno.test("hydrateFile with a namespace named data: writes where core reads (swamp-club#2404)", async () => {
  const cachePath = await Deno.makeTempDir({ prefix: "gcssync-hydrate-dns-" });
  try {
    const mock = createMockGcsClient();
    mock.storage.set(
      "data/data/m/1/raw",
...
async function setupStray(opts: {
  ...
  mock: ReturnType<typeof createMockGcsClient>;
  cachePath: string;
  service: GcsCacheSyncService;
  strayPath: string;
}> {
  const cachePath = await Deno.makeTempDir({ prefix: "gcssync-stray-" });
  const mock = createMockGcsClient();
  mock.storage.set(
    "my-ns/.datastore-index.json",
    encodeIndex({
...
for (const ns of ["files", "data"]) {
  Deno.test(`pushChanged with a namespace named ${ns}: real ${ns}/data/... paths still push (swamp-club#2404)`, async () => {
    const cachePath = await Deno.makeTempDir({ prefix: "gcssync-layout-ns-" });
    try {
      const mock = createMockGcsClient();
      await seedFile(
        cachePath,
...
Deno.test("pullChanged ignores stray index entries pushed by the bug (swamp-club#2404)", async () => {
  const cachePath = await Deno.makeTempDir({ prefix: "gcssync-stray-pull-" });
  try {
    const mock = createMockGcsClient();
