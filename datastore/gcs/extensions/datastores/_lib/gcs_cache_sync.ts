  ...
  private async writeLocalIndexAfterPush(
    ...
    if (!this.indexIsPartial) {
      try {
        ...
        console.warn(
          `[gcs-sync] Index writeback failed, fast path stays unarmed: ${err}`,
        );
        return false;
      }
    }
    try {
      ...
      console.warn(
        `[gcs-sync] Index merge failed, fast path stays unarmed: ${err}`,
      );
      return false;
    }
  }
          ...
          try {
            ...
            if (listPrefixes.length > 0) {
              const remoteListing = (await Promise.all(
                listPrefixes.map((p) =>
                  retryWithBackoff(() => this.gcs.listAllObjects(p, signal), {
                    signal,
                  })
                ),
              )).flat();
                ...
                if (stat.isFile) {
                  ...
                  if (!this.lazyPullActive && this.index) {
                    ...
                    for (const key of Object.keys(this.index.entries)) {
                      ...
                      if (isStrayNamespacePath(key, this.namespace)) continue;
