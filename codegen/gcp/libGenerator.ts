...
async function getCredentials(): Promise<GcpCredentials> {
  ...
  if (directToken) {
    const projectId = Deno.env.get("GCP_PROJECT")?.trim() || Deno.env.get("GOOGLE_CLOUD_PROJECT")?.trim();
    if (!projectId) {
      throw new Error(
...
function isLongRunningOperation(response: any): boolean {
  ...
  if ("done" in response && "name" in response && typeof response.name === "string" && response.name.includes("operations/")) return true;
