...
Use `swamp model type describe @swamp/aws/aps/anomaly_detector` to see the full
...
```bash
# Create a new anomaly_detector model
swamp model create @swamp/aws/aps/anomaly_detector my-anomaly_detector

# Edit the model to configure its properties
swamp model edit my-anomaly_detector

# Create the resource in AWS
swamp model method run my-anomaly_detector create

# Sync current state from AWS
swamp model method run my-anomaly_detector sync
```
