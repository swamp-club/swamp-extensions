...
Use `swamp model type describe @swamp/aws/emrcontainers/endpoint` to see the
...
```bash
# Create a new endpoint model
swamp model create @swamp/aws/emrcontainers/endpoint my-endpoint

# Edit the model to configure its properties
swamp model edit my-endpoint

# Create the resource in AWS
swamp model method run my-endpoint create

# Sync current state from AWS
swamp model method run my-endpoint sync
```
