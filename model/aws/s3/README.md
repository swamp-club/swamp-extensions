...
Use `swamp model type describe @swamp/aws/s3/bucket` to see the full list of
...
```bash
# Create a new bucket model
swamp model create @swamp/aws/s3/bucket my-bucket

# Edit the model to configure its properties
swamp model edit my-bucket

# Create the resource in AWS
swamp model method run my-bucket create

# Sync current state from AWS
swamp model method run my-bucket sync
```
