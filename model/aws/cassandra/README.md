...
Use `swamp model type describe @swamp/aws/cassandra/keyspace` to see the full
...
```bash
# Create a new keyspace model
swamp model create @swamp/aws/cassandra/keyspace my-keyspace

# Edit the model to configure its properties
swamp model edit my-keyspace

# Create the resource in AWS
swamp model method run my-keyspace create

# Sync current state from AWS
swamp model method run my-keyspace sync
```
