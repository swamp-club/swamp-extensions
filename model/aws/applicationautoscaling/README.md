...
AWS APPLICATIONAUTOSCALING resources.
...
Use
`swamp model type describe @swamp/aws/applicationautoscaling/scalable_target` to
...
```bash
# Create a new scalable_target model
swamp model create @swamp/aws/applicationautoscaling/scalable_target my-scalable_target

# Edit the model to configure its properties
swamp model edit my-scalable_target

# Create the resource in AWS
swamp model method run my-scalable_target create

# Sync current state from AWS
swamp model method run my-scalable_target sync
```
