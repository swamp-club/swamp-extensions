...
Use `swamp model type describe @swamp/aws/licensemanager/grant` to see the full
...
```bash
# Create a new grant model
swamp model create @swamp/aws/licensemanager/grant my-grant

# Edit the model to configure its properties
swamp model edit my-grant

# Create the resource in AWS
swamp model method run my-grant create

# Sync current state from AWS
swamp model method run my-grant sync
```
