...
AWS WORKSPACESINSTANCES resources.
...
Use
`swamp model type describe @swamp/aws/workspacesinstances/volume_association` to
...
```bash
# Create a new volume_association model
swamp model create @swamp/aws/workspacesinstances/volume_association my-volume_association

# Edit the model to configure its properties
swamp model edit my-volume_association

# Create the resource in AWS
swamp model method run my-volume_association create

# Sync current state from AWS
swamp model method run my-volume_association sync
```
