...
Use `swamp model type describe @swamp/aws/amazonmq/broker` to see the full list
...
```bash
# Create a new broker model
swamp model create @swamp/aws/amazonmq/broker my-broker

# Edit the model to configure its properties
swamp model edit my-broker

# Create the resource in AWS
swamp model method run my-broker create

# Sync current state from AWS
swamp model method run my-broker sync
```
