...
Use `swamp model type describe @swamp/aws/kinesis/stream` to see the full list
...
```bash
# Create a new stream model
swamp model create @swamp/aws/kinesis/stream my-stream

# Edit the model to configure its properties
swamp model edit my-stream

# Create the resource in AWS
swamp model method run my-stream create

# Sync current state from AWS
swamp model method run my-stream sync
```
